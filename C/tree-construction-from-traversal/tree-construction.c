...
void constructTree(char* inTokens, char* preTokens);
...
int main(int argc, char **argv)
{
  ...
  // declaration of array of pointers to character
  // to store a list of strings.
  char* inTokens[20]; 
  char* preTokens[20]; /* Difference between pointer to an array and array of pointers.
  ref: https://www.geeksforgeeks.org/difference-between-pointer-to-an-array-and-array-of-pointers/
  */
  int inTokIdx = 0;
  int preTokIdx = 0;
  ...
  // generate tree
  constructTree(inTokens, preTokens);
}

struct node* nodealloc(char key)
{
...
void constructTree(char* inTokens[], char* preTokens[])
{
  struct node* root = nodealloc(preTokens[0]);
